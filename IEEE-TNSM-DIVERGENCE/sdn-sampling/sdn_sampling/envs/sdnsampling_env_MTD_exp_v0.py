...
class SdnSamplingEnv_MTD_exp_v0(gym.Env):
    ...
    def step(self, action):
        ...
        if len(self.result_total) == self.num_steps:
            ...
            # save mv results
            with open('./tmp/results/output_MTD_exp_v0_ddpg_ft/step_rewards_mv_total.txt', 'w') as file:
                file.write('\n'.join(map(str, mov_avgs_total)))
            with open('./tmp/results/output_MTD_exp_v0_ddpg_ft/step_rewards_mv_mal_f.txt', 'w') as file:
                file.write('\n'.join(map(str, mov_avgs_mal_f)))
            with open('./tmp/results/output_MTD_exp_v0_ddpg_ft/step_rewards_mv_v.txt', 'w') as file:
                file.write('\n'.join(map(str, mov_avgs_v)))

            # save raw results
            with open('./tmp/results/output_MTD_exp_v0_ddpg_ft/step_rewards_total.txt', 'w') as file:
                file.write('\n'.join(map(str, self.result_total)))
            with open('./tmp/results/output_MTD_exp_v0_ddpg_ft/step_rewards_mal_f.txt', 'w') as file:
                file.write('\n'.join(map(str, self.result_mal_f)))
            with open('./tmp/results/output_MTD_exp_v0_ddpg_ft/step_rewards_v.txt', 'w') as file:
    ...
    def _compute_reward(self, cur_state, cur_action):
        ...
        y_bar = [25000000, 25000000, 25000000, 25000000]
        print(y_bar)
    ...
    def _change_sampling_rate(self, samp_rate_o, mtd_prob, attacked_g):
        ...
        # DDPG with naive MTD
        if int(ids_alerts) > 0:
            scan_success_prob = np.random.random()
