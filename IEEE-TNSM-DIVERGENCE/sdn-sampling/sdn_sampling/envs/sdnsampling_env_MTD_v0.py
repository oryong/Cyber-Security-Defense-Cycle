...
class SdnSamplingEnv_MTD_v0(gym.Env):
    ...
    def step(self, action):
        ...
        if len(self.result_total) == self.num_steps:
            ...
            with open('./tmp/results/output_MTD_v0_ddpg_ft/step_rewards_total.txt', 'w') as file:
                file.write('\n'.join(map(str, mov_avgs_total)))

            with open('./tmp/results/output_MTD_v0_ddpg_ft/step_rewards_mal_f.txt', 'w') as file:
                file.write('\n'.join(map(str, mov_avgs_mal_f)))

            with open('./tmp/results/output_MTD_v0_ddpg_ft/step_rewards_v.txt', 'w') as file:
    ...
    def _compute_reward(self, cur_state, cur_action):
        ...
        for i in range(int(self.num_flows - num_mal), self.num_flows):
            ...
            while np.sum(self.routing_matrix[i]) == 0:
                ...
                if np.random.random() < 0.1:
                    j = np.random.randint(7, self.num_switches)
        ...
        for flow_index in range(self.num_flows - num_mal, self.num_flows):
            if sampled_flows[flow_index] >= .5:
                mal_samp_tmp += 1
                for j in range(6, self.num_switches):
