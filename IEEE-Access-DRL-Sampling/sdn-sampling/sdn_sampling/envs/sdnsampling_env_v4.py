...
class SdnSamplingEnv_v4(gym.Env):
    ...
    def reset(self):
        ...
        while True:
            ...
            if len(init_set_samp_point) == self.num_samp:
                break
        #self.state = [2, 1, 1, 2, 9, 2, 7, 1, 8, 2]
        done = False
    ...
    def step(self, action):
        ...
        if len(self.result_f) == self.num_steps:
            ...
            with open('./tmp/results/output_random/step_rewards_f.txt', 'w') as file:
                file.write('\n'.join(map(str, mov_avgs_f)))
            with open('./tmp/results/output_random/step_rewards_mal_f.txt', 'w') as file:
                file.write('\n'.join(map(str, mov_avgs_mal_f)))
            with open('./tmp/results/output_random/step_rewards_u.txt', 'w') as file:
                file.write('\n'.join(map(str, mov_avgs_u)))
            with open('./tmp/results/output_random/step_rewards_v.txt', 'w') as file:
                file.write('\n'.join(map(str, mov_avgs_v)))
            with open('./tmp/results/output_random/step_mean_overhead.txt', 'w') as file:
                file.write('\n'.join(map(str, mov_avgs_mean_overhead)))
            with open('./tmp/results/output_random/step_rewards_total.txt', 'w') as file:
    ...
    def _compute_reward(self, cur_state):
        ...
        # mal_samp = num_mal * (1 - np.mean(mal_samp_tmp))
